...
function insertEventQuery(event) {
  return QC.INSERT_EVENT.format(event.program_id, event.season_id, getSqlDateString(event.event_date));
}
...
function insertSeasonQuery(season) {
  return QC.INSERT_SEASON.format(season.season, season.year);
}
...
function dbSeed() {
  return Promise.using(getSqlConnection(), (connection) => {
    const querys = [
      ...
      insertSeasonQuery(SC.SEASON_1),
      insertSeasonQuery(SC.SEASON_2),
      insertSeasonQuery(SC.SEASON_3),
