...
const PRE = 'pre';
const POST = 'post';
const PRE_SEASON = PRE + '_season';
const POST_SEASON = POST + '_season';
const DROP_VIEW_IF_EXISTS = 'DROP VIEW IF EXISTS {0}';
const DROP_PRE_SEASON_VIEW = DROP_VIEW_IF_EXISTS.format(PRE_SEASON);
const DROP_POST_SEASON_VIEW = DROP_VIEW_IF_EXISTS.format(POST_SEASON);

const CREATE_VIEW = 'CREATE VIEW {0} AS {1}';
const CREATE_SEASON_VIEW = 'SELECT s.student_id {0}_1, s.first_name {0}_2, s.last_name {0}_3, st.site_name {0}_4, p.program_name {0}_5, ' +
...
const CREATE_PRE_SEASON_VIEW = CREATE_VIEW.format(PRE_SEASON, CREATE_SEASON_VIEW.format(PRE, ''));
const CREATE_POST_SEASON_VIEW = CREATE_VIEW.format(POST_SEASON, CREATE_SEASON_VIEW.format(POST, 'NOT'));

const SELECT_SEASON_CSV = 'SELECT ' +
  ...
  'COALESCE(season.' + PRE + '_4, season.' + POST + '_4) site_name, ' +
  'COALESCE(season.' + PRE + '_5, season.' + POST + '_5) program_name, ' +
  'pre_date, pre_height, pre_weight, pre_pacer, ' +
  'post_date, post_height, post_weight, post_pacer ' +
  'FROM ' +
  '(SELECT * FROM ' + PRE_SEASON + ' t1 LEFT JOIN ' + POST_SEASON + ' t2 ON t1.' + PRE + '_1 = t2.' + POST + '_1 ' +
  'UNION ' +
  'SELECT * FROM ' + PRE_SEASON + ' t1 RIGHT JOIN ' + POST_SEASON + ' t2 ON t1.' + PRE + '_1 = t2.' + POST + '_1) ' +
  'season';
...
module.exports = {
  ...
  DROP_PRE_SEASON_VIEW,
  DROP_POST_SEASON_VIEW,
  CREATE_PRE_SEASON_VIEW,
  CREATE_POST_SEASON_VIEW,
  SELECT_SEASON_CSV,
};
