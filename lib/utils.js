  ...
  return Promise.using(getSqlConnection(), function(connection) {
    ...
    .catch(function(err) {
      if (err.code == 'ER_TRUNCATED_WRONG_VALUE_FOR_FIELD') {
        throw {name: 'InvalidArgumentError', status: 400, message: 'ID type error'};
      }
      if (err.code == 'ER_TRUNCATED_WRONG_VALUE') {
        throw {name: 'InvalidArgumentError', status: 400, message: 'ID type error'};
      }
      if (err.code == 'ER_NO_REFERENCED_ROW_2') {
        throw {name: 'ArgumentNotFound', status: 404, message: 'ID not found in DB'};
      }
      if (err.code == 'ER_BAD_NULL_ERROR') {
        throw {name: 'InvalidArgumentError', status: 400, message: 'Body not found'};
      }
      throw err;
    });
  });
...
module.exports = {
  isValidDate, isPositiveInteger, query, seed
};
